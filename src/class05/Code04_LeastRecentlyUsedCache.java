 ...
 * 内存替换算法
 * 题意：在有限容量K的结构中，
 * 每次添加/修改，查询都记录操作时间，当容量达到k时，再添加一个记录，就把最老的操作时间的记录删掉
 * 要求添加/修改，查询时间复杂度O(1)
 *
 * 思路：采用hashMap+双向链表
 * 将map中的key，value封装成一个节点，用双向链表连起来，双向链表用来模拟操作时间，最近操作过的都在尾部，很久没有访问过的在链表头部
 * 所以当对某条记录进行操作，需要将该记录对应的节点移动到尾部
 * 当容量达到最大值，删掉链表的头，即最久没有访问的记录，再加上新节点
 */
